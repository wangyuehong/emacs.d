...
### AC-0020-0035：上下文内容格式与 code-ref 一致

- Given：上下文配置开启「包含内容」
- When：调用带上下文发送
- Then：代码块内容遵循 code-ref 的含内容格式规则，默认阈值为 3 行，配置阈值不得低于 3 行

### AC-0020-0036：保存后再生成上下文

- Given：当前 file-visiting buffer 已修改
- When：调用带上下文发送
- Then：buffer 先被保存再生成位置与内容，上下文行号基于保存后的 buffer

### AC-0020-0040：非文件缓冲区纯命令发送

- Given：当前 buffer 未关联文件
- When：调用带上下文发送
- Then：仅发送命令正文，不带位置行，不触发保存或内容生成；行为等价于 US-0010
