;;; vbnet-mode.el  --- A mode for editing Visual Basic .NET programs. -*- lexical-binding: t; -*-
;;
...
;;   capitalization; integration with compile.el and imenu.el;
;;   built-in snippets for ya-snippet.el; and some minor convenience
;;   functions.

;; Local modifications for Emacs 31 compatibility (April 2026):
;;   - Added `lexical-binding: t'
;;   - Replaced `defadvice' (imenu) with `define-advice'
;;   - Removed flymake integration (used obsolete flymake API removed in Emacs 26+)
;;   - Removed `window-system' guard in `vbnet-enable-font-lock' (TUI support)
;;   - Replaced obsolete hooks: `local-write-file-hooks' -> `before-save-hook',
;;     `find-file-hooks' -> `find-file-hook',
;;     `pre-abbrev-expand-hook' -> `abbrev-expand-function'
;;   - Fixed `eval-after-load' yasnippet missing quote
;;   - Original source: https://github.com/DinoChiesa/dpchiesa-elisp
...
(define-advice imenu--split-menu (:around (orig-fn menulist title) vbnet-patch)
  "In VB buffers, skip imenu's default submenu-reordering."
  (if (and buffer-file-name
           (string-match "\\.[Vv][Bb]$" (file-relative-name buffer-file-name))
    ...
    (funcall orig-fn menulist title)))
...
It looks for MARKER-STRING followed by a colon and returns the rest
of that line, or nil if not found.  LINE-LIMIT restricts the search
range (positive = from top, negative = from bottom, 0 = no limit)."
...
\\{vbnet-mode-map}"
  ...
  (add-hook 'find-file-hook #'vbnet-load-associated-files nil t)
