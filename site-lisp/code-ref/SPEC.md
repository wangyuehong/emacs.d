...
- When：调用
- Then：剪贴板内容包含位置串，以及用 Markdown 代码围栏包裹的选中/当前行内容

### AC-0020-0035：含内容默认按 3 行阈值自动缩略

- Given：请求 with-content 变体，且内容不超过 3 行
- When：调用
- Then：剪贴板中的代码围栏完整保留选中/当前行内容

### AC-0020-0036：自动缩略阈值可配置

- Given：用户配置自动缩略阈值
- When：阈值合法
- Then：默认阈值为 3 行，且配置值不得低于 3 行

### AC-0020-0037：超过阈值的内容自动缩略

- Given：请求 with-content 变体，且内容行数超过当前自动缩略阈值
- When：调用
- Then：剪贴板中的代码围栏仅保留第一行、最后一行，并在中间用 `[... omitted N lines ...]` 标明省略行数

### AC-0020-0038：缩略不改变位置范围
